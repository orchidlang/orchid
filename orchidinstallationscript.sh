...
# Step 1: Write the Python script to /usr/local/bin/orchid.py
cat << 'EOF' > /usr/local/bin/orchid.py
import os
import sys
...
# Step 2: Create the shell wrapper script
echo '#!/bin/bash' > /usr/local/bin/orchid.sh
echo 'python3 /usr/local/bin/orchid.py "$@"' >> /usr/local/bin/orchid.sh

# Step 3: Make the shell script executable
chmod +x /usr/local/bin/orchid.sh

echo "Orchid installed. Run it using: orchid.sh"
